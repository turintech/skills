...
3. When supported, run `artemis --output-format json capabilities` once. Check exact `command` paths and `flags` in its `commands` array; this reports installed CLI support, not server compatibility or authentication. If unavailable or not a valid JSON capability report, fall back to command-specific help. Confirm support for `changeset download`, `changeset save`, `changeset validate`, and `project scripts`, including the flags needed below. Check `changeset create` only when a new changeset is needed. A zero exit code is insufficient: older CLIs can print parent help for an unsupported subcommand. Parent usage such as `artemis changeset [command]` does not establish support for `changeset save`. If the installed CLI matches the official stable release but lacks a required command, stop and report a CLI release requirement; repeatedly reinstalling the same release will not help. Do not switch to a development channel or the Git workflow without the user choosing it.
