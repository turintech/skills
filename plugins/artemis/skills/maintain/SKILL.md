...
```bash
artemis maintain scans run --project <p> --rule <r> --model <model>   # a model code from artemis model list
```
...
- `--count` (default 5, range 1 to 1000) is the *target* number of issues to
...
shrank); `archive` (formerly `close`) retires an issue without judging it true or false (won't-fix,
duplicate).

## 6. Fix: dispatch the fix agent

```bash
artemis maintain issues fix <issue-id> [<issue-id>...] [--model <model>]
```
